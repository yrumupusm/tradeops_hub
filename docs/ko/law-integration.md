...
관리 화면은 운영 현황(상태·관리 작업·수집 이력 순서), 보유 법령, 검색 로그의 세 탭으로 구성한다. 검색 로그와 선택한 요청의 실행 추적은 같은 너비로 나란히 표시하고 1100px 이하에서는 위아래로 배치한다. 요청 ID를 누르면 행을 강조하고 추적 입력란에 값을 채운 뒤 목록 이동 없이 해당 요청을 조회한다. UUID 직접 조회·초기화, 영역별 로딩·오류와 이전 요청 취소를 지원한다. 선택 전에는 전체 실행 추적을 요청하지 않는다. 기존 API와 운영 책임자 권한은 유지한다.
