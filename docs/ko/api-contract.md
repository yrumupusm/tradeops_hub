...
모든 `/api/...` 엔드포인트는 원본 `law_research_assistant` API 형식과 호환되도록 `/api/v1/...`으로도 제공합니다. `/api/...`는 Spring UI의 camelCase 응답 계약을 유지하고, `/api/v1/...`은 원본 방식의 snake_case 필드명을 반환합니다. 예를 들면 `candidate_laws`, `cited_articles`, `follow_up_questions`, `effective_basis`, `retrieval_stats`, `law_id`, `article_id`, `index_status`, `recent_failures`, `reindex_enabled`가 있습니다.
...
원본 호환 질문 엔드포인트입니다. `as_of`, `research_areas` 별칭 지원을 포함해 요청 검증은 `/api/ask`와 같지만, 응답은 snake_case 필드명과 소문자 enum 값을 사용합니다.
...
`indexStatus`는 완료된 스냅샷과 색인 조문 수를 기준으로 `healthy`, `missing`, `stale` 중 하나를 반환합니다. `stale`은 DB에 저장된 조문 수와 vector index에 올라간 조문 수가 달라 점검이 필요한 상태입니다. 누락과 초과 벡터의 원인을 확인한 후 조치하며, 자동 재색인이 필요하다는 뜻은 아닙니다.
...
`/api/health`와 `/api/v1/health`는 DB와 검색 색인의 준비 상태를 반환합니다.
...
DB 점검이 실패하거나 조문 색인이 missing/stale 상태이면 `status`는 `degraded`입니다.

## GET /healthz

원본 서비스 형식과 호환되는 루트 생존 확인 엔드포인트입니다.
...
가장 최근에 완료된(`INDEXED`) 스냅샷의 법령만 반환합니다. 이전 스냅샷은 조문 이력·비교 엔드포인트에서 계속 조회할 수 있지만 기본 법령 목록에 중복 표시하지 않습니다.
...
`ADMIN_REINDEX_ENABLED=true`가 아니면 이 엔드포인트는 비활성화됩니다. 비활성 상태에서는 `403 Forbidden`과 `error` 메시지를 반환합니다.
`/api/admin/reindex`는 Spring 관리 UI의 동기 응답 계약을 유지합니다. `/api/v1/admin/reindex`는 원본 서비스 형식에 맞춰 `RUNNING` 수집 실행 이력을 만들고, 백그라운드 재색인 작업을 예약한 뒤 즉시 `202 Accepted`를 반환합니다.

`/api/admin/reindex` 응답:
