...
This is a historical record, not current deployment status. See todo.md for the current review.

# Delivery checklist — BIS workspace

## Pre-publication completion review

- [ ] Audit paired documentation against current routes/contracts; resolve obsolete current-state claims and preserve historical notes separately.
- [ ] Diagnose law index mismatch without reembedding; preserve original data and vectors, verify any maintenance with backups.
- [ ] Run isolated monorepo and final runtime gates plus browser acceptance of current UI.
- [ ] Retire the old local checkout safely and record remaining external-publication prerequisites.
- [ ] Commit verified pending UI changes and final review; publish source history without exposing the site.

- [x] Align law administration corpus/trace inputs and query/reset buttons at 40px with bottom alignment. Frontend local gate and production build passed; web switched to .next-law-input-alignment and /law-admin returned HTTP 200. API/data unchanged.
