...
개수 차이만으로 벡터 재생성이 필요하다고 판단하지 않는다. 먼저 현재 DB 조문 ID, 벡터 포인트 ID와 메타데이터의 조문 ID를 비교한다. 유지보수 중에는 데이터 변경 작업을 중지한다. 원래 컬렉션의 Qdrant 스냅샷을 만들고 현재 조문으로 확인된 ID만 같은 벡터 설정의 새 컬렉션에 복사한다. 모든 벡터·메타데이터 해시와 ID 범위가 일치하는지 확인한 후 `VECTOR_COLLECTION`을 변경하고 법령 서버를 재시작한다. 이전 컬렉션·설정은 복구용으로 보존한다. 안내를 없애기 위해 원본 이력 변경 감지나 과거 조문 날짜를 임의로 바꾸지 않는다.

2026-09-11 점검에서는 현재 조문의 누락 없이 DB에 없는 조문을 가리키는 포인트 319개를 확인했다. 현재 벡터 4,112개를 같은 해시로 복사하고 이전 4,431개 컬렉션을 보존했으며 상태는 정상으로 바뀌었다. 이 작업은 수동 유지보수다. 자동 잔여 벡터 정리·본문 해시 기반 최신성 추적은 구현하지 않았다. 개수·ID 일치만으로 본문 최신성을 증명하지는 않으므로 이후 수집·재색인에서도 차이를 점검한다.
